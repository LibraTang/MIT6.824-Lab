...
func (kv *ShardKV) GetShardsData(request *ShardOperationRequest, response *ShardOperationResponse) {
	...
	// the request wants data from future, but not ready for now
...
func (kv *ShardKV) DeleteShardsData(request *ShardOperationRequest, response *ShardOperationResponse) {
	...
	// This is the leader server which needs to delete outdated shards' data, so needs to execute command in here
	// To change status from BePulling to Serving
...
// Get specific status shards' group ids in previous config
...
func (kv *ShardKV) applyOperation(message *raft.ApplyMsg, operation *CommandRequest) *CommandResponse {
	...
	if kv.canServe(shardID) {
		if operation.Op != OpGet && kv.isDuplicateRequest(operation.ClientId, operation.CommandId) {
			DPrintf("{Node %v}{Group %v} doesn't apply duplicated message %v to stateMachine because maxAppliedCommandId is %v for client %v", kv.rf.Me(), kv.gid, message, kv.lastOperations[operation.ClientId].MaxAppliedCommandId, operation.ClientId)
...
func (kv *ShardKV) applyConfiguration(nextConfig *shardctrler.Config) *CommandResponse {
	// the configuration is from shard controller, so there is no need to update lastOperations
...
// The real gc process. Initialize shards to delete outdated data
		...
		go func(servers []string, configNum int, shardIDs []int) {
			...
			for _, server := range servers {
				...
				if srv.Call("ShardKV.GetShardsData", &pullTaskRequest, &pullTaskResponse) && pullTaskResponse.Err == OK {
					DPrintf("{Node %v}{Group %v} gets a PullTaskResponse %v and tries to commit it when currentConfigNum is %v", kv.rf.Me(), kv.gid, pullTaskResponse, kv.currentConfig.Num)
		...
		go func(servers []string, configNum int, shardIDs []int) {
			...
			for _, server := range servers {
				...
				if srv.Call("ShardKV.DeleteShardsData", &gcTaskRequest, &gcTaskResponse) && gcTaskResponse.Err == OK {
					DPrintf("{Node %v}{Group %v} deletes shards %v in remote group successfully when currentConfig is %v", kv.rf.Me(), kv.gid, shardIDs, configNum)
					// To change status from GCing to Serving
